select s.id, s.NAME, s.DESCRIPTION, s.GRADE from SUBJECT s right join MARK M on s.ID = M.STUDENT_ID group by s.id having avg(m.MARK) < (select AVG(MARK) from MARK);
